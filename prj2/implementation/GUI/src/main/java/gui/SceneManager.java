...
class SceneManager {
    ...
    private <T> Parent loadScene(String fxml, Consumer<T> consumer) {
        URL fxmlResource = GUIApp.class.getResource(fxml + ".fxml");
        if (fxmlResource == null) {
            Logger.getLogger(SceneManager.class.getName()).log(Level.SEVERE, "FXML resource not found: " + fxml);
            return createErrorPane(fxmlResource, new IOException("FXML resource not found: " + fxml));
        }
    ...
    Parent createErrorPane(URL fxmlResource, IOException ex) {
        ...
        addRow(parent, "File", fxmlResource != null ? fxmlResource.toString() : "null");
        addRow(parent, "Loader name", loaderName);

        addRow(parent, "Cause class", ex.getCause() != null ? ex.getCause().getClass().toString() : "null");
        addRow(parent, "Cause message", ex.getCause() != null ? ex.getCause().getMessage() : "null");

        var stackTrace = Stream.of(ex.getStackTrace()).limit(10).map(st -> st.toString())
    ...
    void addRow(VBox parent, String label, String text) {
        ...
        row.getChildren().addAll(nameLabel, textLabel);
